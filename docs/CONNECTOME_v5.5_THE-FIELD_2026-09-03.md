...
### 3.2 Chunking [M/V]
- `C:/chunker/chunker.py --budget 700 --overlap 40` (the `--overlap 0` hang is a known defect; keep 40 until fixed). Breadcrumbs (H1 › H2 › H3) become the chunk's deterministic **contextual header**: `{doc title} › {breadcrumb} · {era: YYYY-MM} · {origin}`; the header is embedded with the chunk (contextual retrieval, deterministic tier, $0).
- **Late chunking** when a document is ≤ 32K tokens (the embedder's window): embed the whole document with `--pooling none`, then mean-pool the token vectors inside each chunk's span. Documents longer than 32K tokens fall back to header-only chunk embedding. Bake-off B0-3 decides the default by measured recall on the goldens; the model card and the 2025–26 literature both predict late chunking wins on long documents.
...
- **Decision:** 4B by default (fits beside a 9B reader on 16 GB), 8B for cold builds when the card is free; **0.6B is the floor and the current state.** Stored dims: 1024 (MRL-truncated, re-normalised) as the serving vector, plus a 256-d coarse head. The organ currently embeds queries **without** the instruct prefix; that is a measured-by-the-card 1–5 % loss and a B0 bake-off (F-PREFIX).
...
- **Qwen3-Reranker-0.6B/4B GGUF** on llama-server `/v1/rerank` (`--pooling rank`; the near-zero-score GGUF conversion bug and its fix are documented) [V] gist by VooDisss, 2026. Reranks the fused top-100 to top-20 in ≈ 0.5–2 s locally. Late-interaction token vectors (ColBERT-class) are refused at v5.5 on storage grounds: 700 tokens × 128-d int8 ≈ 90 KB/chunk → 22 GB at 250k chunks; revisit if F-RENT shows the reranker is the bottleneck.
...
- `RRF(k=60)` over dense ∥ BM25 ∥ graph firing (§5.6), then the reranker. **F-RENT:** each slice must lift nDCG@10 on the frozen goldens over BM25 ∥ dense alone by a pre-registered margin or it leaves the hot path (the graph decorates from the bench, as BRAIN §D2 says). The rent is printed per answer as the additive contribution of each slice.
...
| **F-PREFIX** | the instruct prefix is noise | recall@20 with vs without the query instruction on the goldens |
...
Research-agent reports (embeddings & retrieval; agent memory & GraphRAG; geometry & render; models & economics; prior art) are filed under `docs/sota/` as they land and are cited by name where they changed a decision.
