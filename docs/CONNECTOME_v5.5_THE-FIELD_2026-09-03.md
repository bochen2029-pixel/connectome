...
- **M2 · The frozen field. ✓ PASSING (2026-09-03).** Consensus Leiden partition/v1 (`harness/partition.py`), assignment with size caps and a real overflow bucket, the drift meter, and the provenance slice (`harness/provenance.py`). *Gate:* `scripts/gate.ps1 -Milestone M2`. **F-LAYOUT-STABLE settled on the corpus [M]:** under +10 %, +20 % and +40 % growth by arrival time, the frozen partition scores ARI 1.000 with **0 % of members moved**, while re-clustering the grown corpus scores 0.670–0.748 and moves **16.9–21.1 %** of it; the comparison is deliberately unkind to the freeze, since ARI is computed on the chunks both arms share and the re-fit arm gets the same ten-seed consensus rather than a single seed. **Consensus earns its cost:** plain Leiden scored ARI 0.73–0.86 across seeds here, consensus scores 0.895 mean / 0.882 worst, giving 70–73 communities at modularity 0.86 with the runs concurring on 90.7 % of edges. **Drift is the price and it is published:** 0.253 → 0.302 across the three growth steps, which is precisely what a version event fires on. **Provenance:** 4,136 exact-12-gram edges link 213 of 264 documents to the sessions that wrote them, with no model in the path — the only slice that states causation rather than similarity, and the source of the free goldens. Receipt: `harness/README.md`. *Carried to M3:* the super-community level (γ = 0.3), bounded local refinement, and the position ledger, which needs claim extraction rather than clustering.
