...
The operator's intuition that the bus should live in a shared KV trunk is now an active research line with numbers: a radix prefix cache for stateful multi-agent tool calling gives 2.1× per turn on 6-turn and 4.2× median on 35-turn workflows over vLLM/SGLang [V] arXiv:2605.26289 (2026-05); KV-cache TTL scheduling for agents cuts job completion time by more than 8× [V] arXiv:2511.02230; coding agents "repeatedly reuse large prefixes", and prefix-aware eviction yields 2–2.6× fewer evictions and up to 3.5× faster sessions [V] arXiv:2606.16824; persisting a Q4 KV per agent on disk gives 22–136× lower time-to-first-token and 4× more agent contexts than FP16 [V] arXiv:2603.04428; a survey catalogues 18 methods for exchanging KV or hidden states between agents, with no shared benchmark yet [V] arXiv:2606.05711 (2026-06). **Gear one already has the cheap half of this today:** llama-server's host-memory prompt cache (`--cache-ram`, unified KV, slot save/restore to disk) re-prefills a shared prefix in milliseconds (measured 16 ms for a 1,664-token prefix) [M], and llama.cpp master carries DeepSeek V4's built-in DSpark draft heads for speculative decoding. The prefix-sharing economics for ten workers on one 100K-token prefix, input side, within cache TTL [D from the 2026-09-03 price sheets]: Fable 5.1 $10.00 → $1.48; Opus 5 $5.00 → $1.08; Sonnet 5 $2.00 → $0.43; GPT-5.6 Sol $4.00 → $0.86; DeepSeek V4 Flash off-peak $0.22 → $0.03; Kimi K3 $3.00 → $0.57; local ≈ $0 with one 24 s prefill then 16 ms per hit. The swarm's shared skeleton is therefore almost free on every gear; what costs money is the reads, and the residual gate is what rations them.
