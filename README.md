...
**Design of record: [`docs/CONNECTOME_v5.5_THE-FIELD_2026-09-03.md`](docs/CONNECTOME_v5.5_THE-FIELD_2026-09-03.md)** — a self-supervised model of a text stream, general across writers, lawyers, researchers and personal archives: next-document prediction is the training signal, the schema is induced, thresholds are the corpus's own quantiles, nothing gates on human labels; a two-pass int8 scan, a frozen versioned partition, an analytic Lorentz map with radius = resolution, the residual attention loop with fronts and vigilance, the rescan scored against held-out documents, the sentry on Intercom, a native C++/CUDA core and viewer, the DeepSeek harness for workers, a stateless MCP server; every number tagged measured, verified, derived, or bet, with the first measurements (convergence by arrival order, partition instability, δ-hyperbolicity, the instruct prefix) already in its evidence table. The organ below is the v0.1 implementation it supersedes; `NEXT.md` carries the state and the milestone gates are in §13.
