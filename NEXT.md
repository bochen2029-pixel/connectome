...
**Updated 2026-09-03 (final crystallisation): the design of record is `docs/CONNECTOME_v5.5_THE-FIELD_2026-09-03.md` (v4 and v5 skipped by operator directive; v0.1–v0.3 are lineage). It is a self-supervised model of the corpus: the training signal is next-document prediction, thresholds are quantiles of the corpus's own distributions, the schema is induced, no human labels gate anything, the hot path and the viewer are native C++/CUDA in the operator's own build conventions, workers run on the DeepSeek harness, and the build proceeds by machine-checkable milestone gates (§13) with no kickoff document during implementation. Its §12 evidence table already holds the first measured numbers (convergence by arrival order, partition instability, δ-hyperbolicity, the instruct prefix). The recommendation below is kept as the 2026-09-01 record; the reading receipt is `docs/READING-RECEIPT_2026-09-03.md`; the five research reports are under `docs/sota/`.**
